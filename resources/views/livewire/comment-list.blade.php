<div>
            ...
            <div class="w-full flex justify-between">
                ...
                @if(auth()->user()->id === $comment->user_id )
                    <div class="actions flex items-center gap-2">
                        <button type="button" wire:click="editComment({{ $comment->id }})" class="text-primary-500 text-xs hover:text-primary-600 hover:underline">{{ __('Edit') }}</button>
                        <span class="text-gray-300">|</span>
                        <button type="button" wire:click="deleteComment({{ $comment->id }})" class="text-danger-500 text-xs hover:text-danger-600 hover:underline">{{ __('Delete') }}</button>
                    </div>
                @endif
            </div>
    ...
    @if($commentId)
        <div class="mt-4">
            ...
            <button wire:click="updateComment" class="px-3 py-2 bg-primary-500 hover:bg-primary-600 text-white rounded mt-3">Update Comment</button>
        </div>
    @endif
</div>
